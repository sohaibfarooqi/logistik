...
def test_simple_order(webapp, simple_order):
  ...
  """
  response = webapp.get(url_for('custom.fulfill_order', order_id=simple_order.id))
...
def test_order_multiple_items(webapp, order_multi):
  ...
  """
  response = webapp.get(url_for('custom.fulfill_order', order_id=order_multi.id))
  assert response.status_code == 200
...
def test_order_insufficient_stock(webapp, order_multi_error):
  ...
  """
  response = webapp.get(url_for('custom.fulfill_order', order_id=order_multi_error.id))
